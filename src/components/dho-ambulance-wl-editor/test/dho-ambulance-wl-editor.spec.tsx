...
describe('dho-ambulance-wl-editor', () => {
  ...
  let delay = async (milliseconds: number) =>
  ...
  it('buttons shall be of different type', async () => {
    fetchMock.mockResponses([JSON.stringify(sampleEntry), { status: 200 }], [JSON.stringify(sampleConditions), { status: 200 }]);

    const page = await newSpecPage({
      components: [DhoAmbulanceWlEditor],
      html: `<dho-ambulance-wl-editor entry-id="test-entry" ambulance-id="test-ambulance" api-base="http://sample.test/api"></dho-ambulance-wl-editor>`,
    });
